...
(function (window, undefined) {
    ...
    function _toCamel(str) {
        ...
        _each(arr, function (key, val) {
            str +=
                (key > 0) ? val.charAt(0).toUpperCase() + val.substr(1) : val;
        });
        return str;
    }
    ...
    function _toMap(val, delimiter) {
        ...
        _each(arr, function (key, val) {
            if ((match = /^(\d+)\.\.(\d+)$/.exec(val))) {
                for (var i = parseInt(match[1], 10);
                     i <= parseInt(match[2], 10); i++) {
                    map[i.toString()] = true;
                }
            } else {
                map[val] = true;
    ...
    K.options = {
        ...
        htmlTags: {
            ...
            span: [
                'id', 'class', '.color', '.background-color', '.font-size',
                '.font-family', '.background',
                '.font-weight', '.font-style', '.text-decoration',
                '.vertical-align', '.line-height'
            ],
            div: [
                'id', 'class', 'align', '.border', '.margin', '.padding',
                '.text-align', '.color',
            ...
            table: [
                'id', 'class', 'border', 'cellspacing', 'cellpadding', 'width',
                'height', 'align', 'bordercolor',
                '.padding', '.margin', '.border', 'bgcolor', '.text-align',
            ...
            'td,th': [
                'id', 'class', 'align', 'valign', 'width', 'height', 'colspan',
                'rowspan', 'bgcolor',
            ...
            'p,ol,ul,li,blockquote,h1,h2,h3,h4,h5,h6': [
                'id', 'class', 'align', '.text-align', '.color',
                '.background-color', '.font-size', '.font-family',
                '.background',
                '.font-weight', '.font-style', '.text-decoration',
    ...
    _extend(KEvent, {
        init: function (el, event) {
            ...
            if (!self.relatedTarget && self.fromElement) {
                self.relatedTarget =
                    self.fromElement === self.target ? self.toElement
                        : self.fromElement;
            }
            if (self.pageX == null && self.clientX != null) {
                var d = doc.documentElement, body = doc.body;
                self.pageX =
                    self.clientX + (d && d.scrollLeft || body && body.scrollLeft
                                    || 0) - (d && d.clientLeft || body
                                             && body.clientLeft || 0);
                self.pageY =
                    self.clientY + (d && d.scrollTop || body && body.scrollTop
                                    || 0) - (d && d.clientTop || body
                                             && body.clientTop || 0);
            }
            if (!self.which && ((self.charCode || self.charCode === 0)
                    ? self.charCode : self.keyCode)) {
            ...
            if (!self.which && self.button !== undefined) {
                self.which =
                    (self.button & 1 ? 1 : (self.button & 2 ? 3 : (self.button
                                                                   & 4 ? 2
                        : 0)));
            }
            switch (self.which) {
    ...
    function _getAttrList(tag) {
        ...
        while ((match = reg.exec(tag))) {
            var key = (match[1] || match[2] || match[4]
                       || match[6]).toLowerCase(),
    ...
    function _addClassToTag(tag, className) {
        if (/\s+class\s*=/.test(tag)) {
            tag =
                tag.replace(/(\s+class=["']?)([^"']*)(["']?[\s>])/,
                            function ($0, $1, $2, $3) {
                                if ((' ' + $2 + ' ').indexOf(' ' + className
                                                             + ' ') < 0) {
                                    return $2 === '' ? $1 + className + $3 : $1
                                                                             + $2
                                                                             + ' '
                                                                             + className
                                                                             + $3;
    ...
    function _formatHtml(html, htmlTags, urlType, wellFormatted, indentChar) {
        ...
        var fontSizeList = 'xx-small,x-small,small,medium,large,x-large,xx-large'.split(
            ',');
        html =
            html.replace(/(<(?:pre|pre\s[^>]*)>)([\s\S]*?)(<\/pre>)/ig,
                         function ($0, $1, $2, $3) {
            ...
            if (attr !== '') {
                ...
                if (tagName === 'font') {
                    ...
                    _each(attrMap, function (key, val) {
                        ...
                        if (key === 'size') {
                            fontStyleMap['font-size'] =
                                fontSizeList[parseInt(val, 10) - 1] || '';
                            delete attrMap[key];
                        }
                        if (key === 'face') {
                ...
                _each(attrMap, function (key, val) {
                    ...
                    if (htmlTags && key !== 'style' && !htmlTagMap[tagName]['*']
                    ...
                    if (key === 'style' && val !== '') {
                        ...
                        _each(styleMap, function (k, v) {
                            if (htmlTags && !htmlTagMap[tagName].style
                                && !htmlTagMap[tagName]['.' + k]) {
                                delete styleMap[k];
                            }
                        });
                        var style = '';
    ...
    function _queryAll(expr, root) {
        ...
        function cmpTag(tagA, tagB) {
            return tagA === '*' || tagA.toLowerCase() === escape(
                tagB.toLowerCase());
        }
        ...
        function byClass(className, tag, root) {
            ...
            if (root.getElementsByClassName) {
                ...
                els =
                    doc.querySelectorAll((root.nodeName !== '#document'
                        ? root.nodeName + ' ' : '') + tag + '.' + className);
                for (i = 0, len = els.length; i < len; i++) {
                    el = els[i];
        ...
        var isChild = false, part, els, subResults, val, v, i, j, k, length,
            len, l;
    ...
    function _computedCss(el, key) {
        ...
        if (win.getComputedStyle) {
            ...
            val =
                style[camelKey] || style.getPropertyValue(key)
                || el.style[camelKey];
    ...
    _extend(KNode, {
        init: function (node) {
            ...
            for (var i = 0, len = node.length; i < len; i++) {
                if (node[i]) {
                    self[i] =
                        node[i].constructor === KNode ? node[i][0] : node[i];
                    length++;
                }
            }
            self.length = length;
        ...
        css: function (key, val) {
            ...
            if (val === undefined) {
                ...
                return self[0].style[_toCamel(key)] || _computedCss(self[0],
                                                                    key) || '';
            }
            self.each(function () {
        ...
        function extractNodes(parent, frag) {
            ...
            while (node) {
                ...
                if (start >= 0 && incStart <= 0) {
                    incStart =
                        testRange.compareBoundaryPoints(_START_TO_START, range);
                }
                if (incStart >= 0 && incEnd <= 0) {
                    incEnd =
                        testRange.compareBoundaryPoints(_END_TO_END, range);
                }
                if (incEnd >= 0 && end <= 0) {
                ...
                if (start > 0) {
                    if (node.nodeType == 1) {
                        ...
                        if (node == copyRange.startContainer) {
                            ...
                            textNode =
                                splitTextNode(node, 0, copyRange.endOffset);
                        } else {
                            textNode =
                                splitTextNode(node, 0, node.nodeValue.length);
                        }
                        if (isCopy) {
                            try {
                                frag.appendChild(textNode);
    ...
    _extend(KRange, {
        ...
        commonAncestor: function () {
            ...
            var parentsA = getParents(this.startContainer),
                ...
                i = 0, lenA = parentsA.length, lenB = parentsB.length, parentA,
                parentB;
            while (++i) {
        ...
        insertNode: function (node) {
            ...
            if (self.compareBoundaryPoints(_END_TO_END,
                                           self.cloneRange().setEnd(ec, eo))
                >= 1) {
                return self;
            }
        ...
        isControl: function () {
            ...
            return sc.nodeType == 1 && sc === ec && so + 1 === eo && K(
                sc.childNodes[so]).isControl();
        },
        get: function (hasControlRange) {
            ...
            rng.setEndPoint('StartToStart', _getEndRange(range.startContainer,
                                                         range.startOffset));
            rng.setEndPoint('EndToStart',
                            _getEndRange(range.endContainer, range.endOffset));
            return rng;
        },
        html: function () {
        ...
        enlarge: function (toBlock) {
            ...
            function enlargePos(node, pos, isStart) {
                ...
                if (pos === 0) {
                    while (!knode.prev()) {
                        parent = knode.parent();
                        if (!parent || _NOSPLIT_TAG_MAP[parent.name] || !toBlock
                                                                        && parent.isBlock()) {
                            break;
                        }
                        knode = parent;
        ...
        shrink: function () {
            ...
            while (self.startContainer.nodeType == 1 && (child =
                self.startContainer.childNodes[self.startOffset])
            ...
            while (self.endContainer.nodeType == 1 && self.endOffset > 0
        ...
        createBookmark: function (serialize) {
            ...
            startNode.id =
                '__kindeditor_bookmark_start_' + (_BOOKMARK_ID++) + '__';
            if (!self.collapsed) {
                endNode = startNode.cloneNode(true);
                endNode.id =
                    '__kindeditor_bookmark_end_' + (_BOOKMARK_ID++) + '__';
            }
            if (endNode) {
    ...
    function _getRng(doc) {
        ...
        if (_IE && (!rng || (!rng.item && rng.parentElement().ownerDocument
                             !== doc))) {
            return null;
        }
        return rng;
    ...
    function _hasAttrOrCss(knode, map) {
        return _hasAttrOrCssByKey(knode, map, '*') || _hasAttrOrCssByKey(knode,
                                                                         map);
    }
    ...
    _extend(KCmd, {
        ...
        select: function (hasDummy) {
            ...
            if (hasDummy && sc.nodeType == 1 && range.collapsed) {
                ...
                if (_WEBKIT) {
                    ...
                    if (K(sc).isInline() || so > 0 && K(children[so - 1])
                            .isInline() || children[so] && K(children[so])
                            .isInline()) {
        ...
        wrap: function (val) {
            ...
            var bookmark = range.createBookmark(),
                ancestor = range.commonAncestor(), isStart = false;
        ...
        commonNode: function (map) {
            ...
            var ec = range.endContainer, eo = range.endOffset,
                node = (ec.nodeType == 3 || eo === 0) ? ec : ec.childNodes[eo
                                                                           - 1];
        ...
        commonAncestor: function (tagName) {
            var range = this.range,
                sc = range.startContainer, so = range.startOffset,
                ec = range.endContainer, eo = range.endOffset,
                startNode = (sc.nodeType == 3 || so === 0) ? sc
                    : sc.childNodes[so - 1],
                endNode = (ec.nodeType == 3 || eo === 0) ? ec : ec.childNodes[eo
                                                                              - 1];
        ...
        createlink: function (url, type) {
            ...
            var html = '<a href="' + _escape(url) + '" data-ke-src="' + _escape(
                url) + '" ';
            if (type) {
    ...
    function _drag(options) {
        var moveEl = options.moveEl,
            ...
            iframeFix = options.iframeFix === undefined ? true
                : options.iframeFix;
        var docs = [document];
        if (iframeFix) {
            ...
            if (options.autoScroll) {
                if (_IE && _V < 7 || _QUIRKS) {
                    ...
                    K(self.win).bind('scroll', function (e) {
                        ...
                        self.pos(_removeUnit(self.x) + diffX, _removeUnit(
                            self.y) + diffY, false);
                    });
                } else {
    ...
    _extend(KEdit, KWidget, {
        init: function (options) {
            ...
            var themesPath = _undef(options.themesPath, ''),
                ...
                isDocumentDomain = location.host.replace(/:\d+/, '')
                                   !== document.domain,
                srcScript = ('document.open();' +
                             (isDocumentDomain ? 'document.domain="'
                             + document.domain + '";' : '') +
                             'document.close();'),
                iframeSrc = _IE ? ' src="javascript:void(function(){'
                                  + encodeURIComponent(srcScript) + '}())"'
                    : '';
            self.iframe =
                K('<iframe class="ke-edit-iframe" hidefocus="true" frameborder="0"'
                  + iframeSrc + '></iframe>').css('width', '100%');
            self.textarea =
                K('<textarea class="ke-edit-textarea" hidefocus="true"></textarea>')
                    .css('width', '100%');
            if (self.width) {
            ...
            function ready() {
                ...
                doc.write(
                    _getInitHtml(themesPath, bodyClass, cssPath, cssData));
                doc.close();
        ...
        html: function (val, isFull) {
            ...
            if (self.designMode) {
                ...
                if (val === undefined) {
                    if (isFull) {
                        val =
                            '<!doctype html><html>' + body.parentNode.innerHTML
                            + '</html>';
                    } else {
                        val = body.innerHTML;
                    }
            ...
            self.div.mouseover(function (e) {
                ...
                .click(function (e) {
                    ...
                    if (knode) {
                        ...
                        self.options.click.call(this, e,
                                                knode.attr('data-name'));
                    }
                });
        },
        get: function (name) {
            ...
            return (this._itemMap[name] =
                K('span.ke-icon-' + name, this.div).parent());
        },
    ...
    _extend(KMenu, KWidget, {
        ...
        addItem: function (item) {
            ...
            if (self.centerLineMode) {
                centerDiv =
                    K('<div class="ke-inline-block ke-menu-item-center"></div>');
                if (height) {
            ...
            if (iconClass !== '') {
                leftDiv.html('<span class="ke-inline-block ke-toolbar-icon ke-toolbar-icon-url '
                             + iconClass + '"></span>');
            }
    ...
    _extend(KColorPicker, KWidget, {
        ...
        _addAttr: function (cell, color, cls) {
            ...
            if (color) {
                cell.append(
                    K('<div class="ke-colorpicker-cell-color" unselectable="on"></div>')
    ...
    _extend(KUploadButton, {
        init: function (options) {
            var self = this,
                ...
                target = options.target || 'kindeditor_upload_iframe_'
                         + new Date().getTime();
            ...
            self.button = button;
            self.iframe =
                options.target ? K('iframe[name="' + target + '"]') : K(
                    'iframe', div);
        ...
        submit: function () {
            ...
            iframe.bind('load', function () {
                ...
                try {
                    ...
                    self.options.afterError.call(self, '<!doctype html><html>'
    ...
    function _createButton(arg) {
        arg = arg || {};
        var name = arg.name || '',
            span = K('<span class="ke-button-common ke-button-outer" title="'
                     + name + '"></span>'),
            btn = K('<input class="ke-button-common ke-button" type="button" value="'
                    + name + '" />');
        if (arg.click) {
            btn.click(arg.click);
        }
    ...
    _extend(KDialog, KWidget, {
        init: function (options) {
            ...
            if (_IE && _V < 7) {
                self.iframeMask =
                    K('<iframe src="about:blank" class="ke-dialog-shadow"></iframe>')
                        .appendTo(self.div);
            } else if (shadowMode) {
            ...
            self.closeIcon =
                K('<span class="ke-dialog-icon-close" title="' + closeBtn.name
                  + '"></span>').click(closeBtn.click);
            ...
            if (showMask) {
                var docEl = _docElement(self.doc),
                    ...
                    docHeight = Math.max(docEl.scrollHeight,
                                         docEl.clientHeight);
                self.mask = _widget({
                                        x: 0,
                                        y: 0,
        ...
        showLoading: function (msg) {
            ...
            self.loading =
                K('<div class="ke-dialog-loading"><div class="ke-inline-block ke-dialog-loading-content" style="margin-top:'
                  + Math.round(body.height() / 3) + 'px;">' + msg
                  + '</div></div>')
    ...
    function _loadScript(url, fn) {
        var head = document.getElementsByTagName('head')[0] || (_QUIRKS
    ...
    function _loadStyle(url) {
        ...
        for (var i = 0, len = links.length; i < len; i++) {
            if (_chopQuery(_formatUrl(links[i].href, 'absolute'))
                === absoluteUrl) {
                return;
            }
        }
    ...
    function _addBookmarkToStack(stack, bookmark) {
        ...
        if (_removeBookmarkTag(bookmark.html) !== _removeBookmarkTag(
                prev.html)) {
            stack.push(bookmark);
        }
    }
    ...
    KEditor.prototype = {
        ...
        updateState: function () {
            ...
            _each(
                ('justifyleft,justifycenter,justifyright,justifyfull,insertorderedlist,insertunorderedlist,'
                 +
                 'subscript,superscript,bold,italic,underline,strikethrough').split(
                    ','), function (i, name) {
        ...
        create: function () {
            ...
            var width = fullscreenMode ? _docElement().clientWidth + 'px'
                : self.width,
                height = fullscreenMode ? _docElement().clientHeight + 'px'
                    : self.height;
            if ((_IE && _V < 8) || _QUIRKS) {
            ...
            K.each(self.items, function (i, name) {
                if (name == '|') {
                    ...
                    htmlList.push('<span class="ke-outline" data-name="' + name
                                  ...
                                  + '" unselectable="on">');
                    htmlList.push('<span class="ke-toolbar-icon ke-toolbar-icon-url ke-icon-'
                                  + name
        ...
        resize: function (width, height, updateProp) {
            ...
            if (height) {
                ...
                editHeight =
                    _removeUnit(height) - self.toolbar.div.height()
                    - self.statusbar.height();
                editHeight =
                    editHeight < self.minHeight ? self.minHeight : editHeight;
        ...
        html: function (val) {
            var self = this;
            if (val === undefined) {
                return self.isCreated ? self.edit.html() : _elementVal(
                    self.srcElement);
            }
        ...
        exec: function (key) {
            ...
            var self = this, cmd = self.cmd,
                changeFlag = _inArray(key,
                                      'selectall,copy,paste,print'.split(','))
                             < 0;
            if (changeFlag) {
        ...
        addBookmark: function (checkSize) {
            ...
            if (checkSize && self._undoStack.length > 0) {
                ...
                if (Math.abs(html.length - _removeBookmarkTag(prev.html).length)
                    < self.minChangeSize) {
                    return self;
                }
            }
        ...
        fullscreen: function (bool) {
            this.fullscreenMode =
                (bool === undefined ? !this.fullscreenMode : bool);
    ...
    function _create(expr, options) {
        ...
        options.themesPath =
            _undef(options.themesPath, options.basePath + 'themes/');
        options.langPath = _undef(options.langPath, options.basePath + 'lang/');
        options.pluginsPath =
            _undef(options.pluginsPath, options.basePath + 'plugins/');
    ...
    _plugin('core', function (K) {
            ...
            _each(self.lang('fontname.fontName'), function (key, val) {
                menu.addItem({
                                 title: '<span style="font-family: ' + key
                                        + ';" unselectable="on">' + val
                                        + '</span>',
                                 checked: (curVal === key.toLowerCase()
                                           || curVal === val.toLowerCase()),
                                 click: function () {
        ...
        self.plugin.getSelectedMedia = function () {
            return _getImageFromRange(self.edit.cmd.range, function (img) {
                return img[0].className == 'ke-media' || img[0].className
                       == 'ke-rm';
            });
        };
        ...
        _each(
            ('prop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,rowmerge,colmerge,'
             +
             'rowsplit,colsplit,coldelete,rowdelete,insert,delete').split(','),
            function (i, val) {
        ...
        _each(
            ('selectall,justifyleft,justifycenter,justifyright,justifyfull,insertorderedlist,'
             ...
             +
             'bold,italic,underline,strikethrough,removeformat,unlink').split(
                ','), function (i, name) {
                if (shortcutKeys[name]) {
                    self.afterCreate(function () {
                        _ctrl(this.edit.doc, shortcutKeys[name], function () {
        ...
        self.afterCreate(function () {
            ...
            function movePastedData() {
                ...
                if (self.pasteType === 2) {
                    html = html.replace(/(<(?:p|p\s[^>]*)>) *(<\/p>)/ig, '');
                    if (/schemas-microsoft-com|worddocument|mso-\w+/i.test(
                            html)) {
                        html =
                            _clearMsWord(html, self.filterMode ? self.htmlTags
                                : K.options.htmlTags);
                    } else {
                        html =
                            _formatHtml(html,
            ...
            return html.replace(
                ...
                .replace(/<img[^>]*class="?ke-(flash|rm|media)"?[^>]*>/ig,
                         function (full) {
                             ...
                             attrs.width =
                                 _undef(imgAttrs.width,
                                        _removeUnit(_undef(styles.width, '')));
                             attrs.height =
                                 _undef(imgAttrs.height,
                ...
                .replace(/<img[^>]*class="?ke-anchor"?[^>]*>/ig,
                         function (full) {
                             ...
                             return '<a name="' + unescape(
                                 imgAttrs['data-ke-name']) + '"></a>';
                         })
                .replace(
                    /<div\s+[^>]*data-ke-script-attr="([^"]*)"[^>]*>([\s\S]*?)<\/div>/ig,
                    function (full, attr, code) {
                        return '<script' + unescape(attr) + '>' + unescape(code)
                               + '</script>';
                    })
                .replace(
                    /<div\s+[^>]*data-ke-noscript-attr="([^"]*)"[^>]*>([\s\S]*?)<\/div>/ig,
                    function (full, attr, code) {
                        return '<noscript' + unescape(attr) + '>' + unescape(
                            code) + '</noscript>';
                    })
                .replace(/(<[^>]*)data-ke-src="([^"]*)"([^>]*>)/ig,
                         function (full, start, src, end) {
                             full =
                                 full.replace(/(\s+(?:href|src)=")[^"]*(")/i,
                                              function ($0, $1, $2) {
                                                  return $1 + _unescape(src)
                                                         + $2;
                                              });
    ...
    self.clickToolbar(name, function () {
        var html = ['<div style="padding:10px 20px;">',
            ...
            // right start
                    ...
                    '</div>',
                    '<div class="ke-map" style="width:' + mapWidth
                    + 'px;height:' + mapHeight + 'px;"></div>',
                    '</div>'].join('');
        var dialog = self.createDialog({
                                           name: name,
                                           width: mapWidth + 42,
...
KindEditor.plugin('emoticons', function (K) {
    var self = this, name = 'emoticons',
        ...
        allowPreview = self.allowPreviewEmoticons === undefined ? true
    ...
    self.clickToolbar(name, function () {
        ...
        if (allowPreview) {
            ...
            previewImg =
                K('<img class="ke-preview-img" src="' + path + startNum
                  + '.gif" />');
        ...
        function bindCellEvent(cell, j, num) {
            ...
            cell.click(function (e) {
                self.insertHtml('<img src="' + path + num
                                + '.gif" border="0" alt="" />').hideMenu()
                    .focus();
                e.stop();
            });
        }
...
KindEditor.plugin('filemanager', function (K) {
    var self = this, name = 'filemanager',
        fileManagerJson = K.undef(self.fileManagerJson, self.basePath
    ...
    function bindTitle(el, data) {
        if (data.is_dir) {
            ...
            el.attr('title',
                    makeFileTitle(data.filename, data.filesize, data.datetime));
        }
    }
    ...
    self.plugin.filemanagerDialog = function (options) {
        var width = K.undef(options.width, 650),
            ...
            viewType = K.undef(options.viewType, 'VIEW').toUpperCase(), // "LIST"
                                                                        // or
                                                                        // "VIEW"
        ...
        function reloadPage(path, order, func) {
            ...
            K.ajax(
                K.addParam(fileManagerJson, param + '&' + new Date().getTime()),
                function (data) {
                    dialog.hideLoading();
                    func(data);
                });
        ...
        function bindEvent(el, result, data, createFunc) {
            var fileUrl = K.formatUrl(result.current_url + data.filename,
                'absolute'),
                dirPath = encodeURIComponent(result.current_dir_path
                                             + data.filename + '/');
            if (data.is_dir) {
                el.click(function (e) {
        ...
        function createCommon(result, createFunc) {
            ...
            if (result.current_dir_path) {
                moveupLink.click(function (e) {
                    reloadPage(result.moveup_dir_path, orderTypeBox.val(),
                               createFunc);
                });
            }
            ...
            function changeFunc() {
                if (viewTypeBox.val() == 'VIEW') {
                    reloadPage(result.current_dir_path, orderTypeBox.val(),
                               createView);
                } else {
                    reloadPage(result.current_dir_path, orderTypeBox.val(),
                               createList);
        ...
        function createList(result) {
            ...
            for (var i = 0, len = fileList.length; i < len; i++) {
                ...
                var iconUrl = imgPath + (data.is_dir ? 'folder-16.gif'
                    : 'file-16.gif'),
                    img = K('<img src="' + iconUrl
                            + '" width="16" height="16" alt="' + data.filename
                            + '" align="absmiddle" />'),
                    cell0 = K(row[0].insertCell(0)).addClass('ke-cell ke-name')
                        .append(img)
                        .append(document.createTextNode(' ' + data.filename));
                ...
                K(row[0].insertCell(2)).addClass('ke-cell ke-datetime')
                    .html(data.datetime);
            }
        }
        ...
        function createView(result) {
            ...
            for (var i = 0, len = fileList.length; i < len; i++) {
                ...
                var fileUrl = result.current_url + data.filename,
                    iconUrl = data.is_dir ? imgPath + 'folder-64.gif'
                        : (data.is_photo ? fileUrl : imgPath + 'file-64.gif');
                var img = K('<img src="' + iconUrl
                            + '" width="80" height="80" alt="' + data.filename
                            + '" />');
                if (!data.is_dir || data.has_file) {
                ...
                photoDiv.append(img);
                div.append('<div class="ke-name" title="' + data.filename + '">'
                           + data.filename + '</div>');
            }
        }
    ...
    self.plugin.flash = {
        edit: function () {
            var html = [
                ...
                //url
                '<div class="ke-dialog-row">',
                '<label for="keUrl" style="width:60px;">' + lang.url
                + '</label>',
                ...
                //width
                '<div class="ke-dialog-row">',
                '<label for="keWidth" style="width:60px;">' + lang.width
                + '</label>',
                '<input type="text" id="keWidth" class="ke-input-text ke-input-number" name="width" value="550" maxlength="4" /> ',
                '</div>',
                //height
                '<div class="ke-dialog-row">',
                '<label for="keHeight" style="width:60px;">' + lang.height
                + '</label>',
            ...
            var dialog = self.createDialog({
                                               ...
                                               yesBtn: {
                                                   ...
                                                   click: function (e) {
                                                       ...
                                                       var html = K.mediaImg(self.themesPath
                                                                             + 'common/blank.gif',
                                                           {
                                                               src: url,
                                                               type: K.mediaType(
            ...
            if (img) {
                ...
                widthBox.val(K.removeUnit(img.css('width')) || attrs.width
                             || 0);
                heightBox.val(K.removeUnit(img.css('height')) || attrs.height
                              || 0);
            }
            urlBox[0].focus();
            urlBox[0].select();
...
KindEditor.plugin('image', function (K) {
    ...
    self.plugin.imageDialog = function (options) {
        ...
        var hiddenElements = [];
        for (var k in extraParams) {
            hiddenElements.push('<input type="hidden" name="' + k + '" value="'
                                + extraParams[k] + '" />');
        }
        var html = [
            '<div style="padding:20px;">',
            ...
            //url
            '<div class="ke-dialog-row">',
            '<label for="remoteUrl" style="width:60px;">' + lang.remoteUrl
            + '</label>',
            ...
            //size
            '<div class="ke-dialog-row">',
            '<label for="remoteWidth" style="width:60px;">' + lang.size
            + '</label>',
            lang.width
            ...
            + imgPath + 'align_left.gif" width="23" height="25" alt="" />',
            ' <input type="radio" name="align" class="ke-inline-block" value="right" /> <img name="rightImg" src="'
            + imgPath + 'align_right.gif" width="23" height="25" alt="" />',
            '</div>',
            //title
            '<div class="ke-dialog-row">',
            '<label for="remoteTitle" style="width:60px;">' + lang.imgTitle
            + '</label>',
            ...
            '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="'
            + target + '" action="' + K.addParam(uploadJson, 'dir=image')
            + '">',
            //file
            '<div class="ke-dialog-row">',
            hiddenElements.join(''),
        ...
        refreshBtn.click(function (e) {
            var tempImg = K('<img src="' + urlBox.val() + '" />', document)
                .css({
                         position: 'absolute',
                         visibility: 'hidden',
                         top: 0,
        ...
        widthBox.change(function (e) {
            if (originalWidth > 0) {
                heightBox.val(
                    Math.round(originalHeight / originalWidth * parseInt(
                        this.value, 10)));
            }
        });
        heightBox.change(function (e) {
            if (originalHeight > 0) {
                widthBox.val(
                    Math.round(originalWidth / originalHeight * parseInt(
                        this.value, 10)));
            }
        });
...
KindEditor.plugin('insertfile', function (K) {
    ...
    self.plugin.fileDialog = function (options) {
        ...
        var html = [
            ...
            //title
            '<div class="ke-dialog-row">',
            '<label for="keTitle" style="width:60px;">' + lang.title
            + '</label>',
...
KindEditor.plugin('lineheight', function (K) {
    ...
    self.clickToolbar(name, function () {
        var curVal = '',
            commonNode = self.cmd.commonNode({'*': '.line-height'});
        if (commonNode) {
        ...
        K.each(lang.lineHeight, function (i, row) {
            K.each(row, function (key, val) {
                menu.addItem({
                                 ...
                                 checked: curVal === key,
                                 click: function () {
                                     self.cmd.toggle('<span style="line-height:'
                                                     + key + ';"></span>', {
                                                                               span: '.line-height='
                                                                                     + key
                                                                           });
                                                   ...
                                                   click: function (e) {
                                                       ...
                                                       var html = K.mediaImg(self.themesPath
                                                                             + 'common/blank.gif',
                                                           {
                                                               ...
                                                               width: width,
                                                               height: height,
                                                               autostart: autostartBox[0].checked
                                                                   ? 'true'
                                                                   : 'false',
                                                               loop: 'true'
                                                           });
    ...
    K.extend(KSWFUpload, {
        init: function (options) {
            ...
            function showError(itemDiv, msg) {
                ...
                K('.ke-message', itemDiv).addClass('ke-error').show()
                    .html(K.escape(msg));
            }
            ...
            var settings = {
                ...
                button_placeholder: K('.ke-swfupload-button > input',
                                      self.div)[0],
                ...
                upload_progress_handler: function (file, bytesLoaded,
                    ...
                    progressbar.bar.css('width', Math.round(percent * 80 / 100)
                                                 + 'px');
                ...
                upload_success_handler: function (file, serverData) {
                    ...
                    try {
                        ...
                        self.options.afterError.call(
                            this, '<!doctype html><html>' + serverData
                                  + '</html>');
                    }
                    if (data.error !== 0) {
                        showError(itemDiv, K.DEBUG ? data.message
        ...
        appendFile: function (file) {
            ...
            var img = K('<img src="' + file.url
                        + '" class="ke-img" data-status="' + file.filestatus
                        + '" width="80" height="80" alt="' + file.name
                        + '" />');
            photoDiv.append(img);
            K('<span class="ke-delete"></span>').appendTo(photoDiv)
                .click(function () {
            ...
            K(['<div class="ke-progressbar">',
               ...
               '<div class="ke-progressbar-percent">0%</div></div>'].join(''))
                .hide().appendTo(statusDiv);
            // message
            K('<div class="ke-message">' + self.options.pendingMessage
...
KindEditor.plugin('multiimage', function (K) {
    ...
    self.plugin.multiImageDialog = function (options) {
        ...
        var dialog = self.createDialog({
                                           ...
                                           beforeRemove: function () {
                                               // IE9 bugfix:
                                               // https://github.com/kindsoft/kindeditor/issues/72
                                               if (!K.IE || K.V <= 8) {
                                                   swfupload.remove();
                                               }
                                           }
        ...
        var swfupload = K.swfupload({
                                        ...
                                        buttonImageUrl: imgPath + (self.langType
                                            ...
                                            : 'select-files-en.png'),
                                        buttonWidth: self.langType == 'zh_CN'
                                            ? 72 : 88,
                                        buttonHeight: 23,
                                        fileIconUrl: imgPath + 'image.png',
                                        uploadDesc: uploadDesc,
                                        startButtonValue: lang.startUpload,
                                        uploadUrl: K.addParam(uploadJson,
                                                              'dir=image'),
 ...
 *
 * SWFUpload is (c) 2006-2007 Lars Huring, Olov Nilz閚 and Mammon Media and is
...
// absolute URL using the current site. Only processes the URL if it can,
...
// settings are set, getting a default value if one was not assigned.
    SWFUpload.prototype.initSettings = function () {
        ...
        if (!this.settings.preserve_relative_urls) {
            //this.settings.flash_url =
            // SWFUpload.completeURL(this.settings.flash_url);	// Don't need
            // to do this one since flash doesn't look at it
...
// to the document
    SWFUpload.prototype.getFlashHTML = function () {
        ...
        // http://www.alistapart.com/articles/flashsatay Fix bug for IE9
...
// added by SWFUpload The element is cached after the first lookup
...
// setting object and joins them up in to a string formatted
...
// strives to remove all references to the SWF, and other objects so memory is
// properly freed. Returns true if everything was destroyed. Returns a false if
// a failure occurs leaving SWFUpload in an inconsistant state. Credits: Major
// improvements provided by steffen
    SWFUpload.prototype.destroy = function () {
        try {
            ...
            if (movieElement && typeof(movieElement.CallFunction)
                ...
                // Loop through all the movie's properties and remove all
                // function references (DOM/JS IE 6/7 memory leak workaround)
...
// combined length of the file names. If the selection name length is too long
// the dialog will fail in an unpredictable manner.  There is no work-around
// for this bug.
...
// the file ID or index. If you do not specify a fileID the current uploading
// file or first file in the queue is cancelled. If you do not want the
// uploadError event to trigger you can specify false for the triggerErrorEvent
// parameter.
...
// beginning of the queue. If nothing is currently uploading then nothing
// happens.
...
// are queued using a setTimeout we must queue them in order to garentee that
...
// objects passed through ExternalInterface cannot have properties that contain
// characters that are not valid for JavaScript identifiers. To work around
// this the Flash Component escapes the parameter names and we must unescape
            ...
            for (var k in file.post) {
                if (file.post.hasOwnProperty(k)) {
                    ...
                    while ((match = reg.exec(uk)) !== null) {
                        uk =
                            uk.replace(match[0], String.fromCharCode(
                                parseInt("0x" + match[1], 16)));
                    }
...
// modify this. Use the swfupload_loaded_handler event setting to execute
// custom code when SWFUpload has loaded.
...
// leaks in IE. This function is called by Flash each time the
// ExternalInterface functions are created.
    SWFUpload.prototype.cleanUp = function (movieElement) {
        // Pro-actively unhook all the Flash functions
        try {
            if (this.movieElement && typeof(movieElement.CallFunction)
                                     === "unknown") { // We only want to do this in IE
                this.debug(
        ...
        // page it doesn't display errors.
    ...
    SWFUpload.prototype.returnUploadStart = function (file) {
        ...
        // upload_start_handler it is interpretted as 'true'.
    ...
    SWFUpload.prototype.uploadSuccess =
        function (file, serverData, responseReceived) {
            ...
            this.queueEvent("upload_success_handler",
                            [file, serverData, responseReceived]);
        };
...
// debug messages call the debug() function.  When overriding the function your
// own function should check to see if the debug setting is true before
    ...
    if (typeof(SWFUpload) === "function") {
        ...
        SWFUpload.queue.uploadStartHandler = function (file) {
            ...
            if (typeof(this.queueSettings.user_upload_start_handler)
                === "function") {
                returnValue =
                    this.queueSettings.user_upload_start_handler.call(this,
                                                                      file);
            }
        ...
        SWFUpload.queue.uploadCompleteHandler = function (file) {
            ...
            if (typeof(user_upload_complete_handler) === "function") {
                continueUpload =
                    (user_upload_complete_handler.call(this, file) === false)
                        ? false : true;
            } else if (file.filestatus === SWFUpload.FILE_STATUS.QUEUED) {
                // If the file was stopped and re-queued don't restart the
                // upload
    ...
    self.clickToolbar(name, function () {
        ...
        while (child) {
            ...
            if (!firstChild || firstChild.name != 'img') {
                ...
                if (!next || (blockMap[next.name] || blockMap[child.name]
                    && !blockMap[next.name])) {
                    if (subList.length > 0) {
    ...
    self.plugin.table = {
        ...
        prop: function (isInsert) {
            var html = [
                ...
                //rows, cols
                '<div class="ke-dialog-row">',
                '<label for="keRows" style="width:90px;">' + lang.cells
                + '</label>',
                lang.rows
                ...
                //width, height
                '<div class="ke-dialog-row">',
                '<label for="keWidth" style="width:90px;">' + lang.size
                + '</label>',
                lang.width
                ...
                //space, padding
                '<div class="ke-dialog-row">',
                '<label for="kePadding" style="width:90px;">' + lang.space
                + '</label>',
                lang.padding
                ...
                //align
                '<div class="ke-dialog-row">',
                '<label for="keAlign" style="width:90px;">' + lang.align
                + '</label>',
                ...
                //border
                '<div class="ke-dialog-row">',
                '<label for="keBorder" style="width:90px;">' + lang.border
                + '</label>',
                lang.borderWidth
            ...
            if (table) {
                ...
                colsBox.val(
                    table[0].rows.length > 0 ? table[0].rows[0].cells.length
                        : 0);
                ...
                if (tableWidth !== undefined && (match =
                        /^(\d+)((?:px|%)*)$/.exec(tableWidth))) {
                    widthBox.val(match[1]);
                    widthTypeBox.val(match[2]);
                } else {
                    widthBox.val('');
                }
                if (tableHeight !== undefined && (match =
                        /^(\d+)((?:px|%)*)$/.exec(tableHeight))) {
                ...
                borderBox.val(
                    table[0].border === undefined ? '' : table[0].border);
                _setColor(colorBox.eq(0),
                          K.toHex(table.attr('borderColor') || ''));
                _setColor(colorBox.eq(1), K.toHex(table[0].style.backgroundColor
                                                  || table[0].bgColor || ''));
                widthBox[0].focus();
                widthBox[0].select();
            }
        ...
        cellprop: function () {
            ...
            _setColor(colorBox.eq(1),
                      K.toHex(cell[0].style.backgroundColor || ''));
            widthBox[0].focus();
            widthBox[0].select();
        },
        insert: function () {
...
KindEditor.plugin('template', function (K) {
    ...
    function getFilePath(fileName) {
        return htmlPath + fileName + '?ver=' + encodeURIComponent(
            K.DEBUG ? K.TIME : K.VERSION);
    }
